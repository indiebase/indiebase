...
export function camelCase(str, { upperCase = false } = {}) {
  if (str.length === 0) {
    return str;
  }
...
export function snakeCaseMappers(opt = {}) {
  return {
...
export function knexIdentifierMappers({
  parse,
  format,
  idSeparator = ':',
}: any = {}) {
...
export function knexIdentifierMapping(colToProp) {
...
export const camelCaseKeys = keyMapper(memoize(camelCase));
export const snakeCaseKeys = keyMapper(memoize(snakeCase));
