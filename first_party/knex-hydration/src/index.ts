...
class ValueType<A> extends Type<A> {
  ...
  constructor(
    ...
    private option: TransformOption,
  ) {
    super();
  }
...
class ArrayType<C extends Type<any>> extends Type<TypeOf<C>[]> {
...
class TypeType<P extends Props> extends Type<{ [K in keyof P]: TypeOf<P[K]> }> {
  constructor(private readonly value: P) {
    super();
  }
...
class NullableType<P extends Props> extends Type<
...
export const number = (field: string, option?: TransformOption): Type<number> =>
  ValueType.of<number>(field, option);
export const string = (field: string, option?: TransformOption): Type<string> =>
  ValueType.of<string>(field, option);
export const date = (field: string, option?: TransformOption): Type<Date> =>
  ValueType.of<Date>(field, option);
export const boolean = (
  field: string,
  option?: TransformOption,
): Type<boolean> => ValueType.of<boolean>(field, option);
export const nullableNumber = (field: string) =>
  ValueType.of<number | null>(field, { id: false });
export const nullableString = (field: string) =>
  ValueType.of<string | null>(field, { id: false });
export const nullableDate = (field: string) =>
  ValueType.of<Date | null>(field, { id: false });
