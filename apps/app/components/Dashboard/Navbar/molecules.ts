...
const providerHashMap = new Map<
  string,
  ReturnType<typeof createIndexedDBProvider>
>();
...
export const WorkspaceMolecule = molecule((_, getScope) => {
  const { defaultWorkspace, defaultPage, defaultMode } =
    getScope(WorkspaceScope);
  const editorContainerAtom = atom<Promise<typeof EditorContainer>>(
    async () => {
  ...
  const currentPageMetaAtom = atomWithObservable<PageMeta[]>((get) => {
