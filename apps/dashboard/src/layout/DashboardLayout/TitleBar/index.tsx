...
import { gridSpacing } from '@/common';

// assets
// import IconSitemap from '@mui/icons-material/AccountTreeTwoTone';
...
export const TitleBar: FC<TitleBarProps> = ({
    ...
    if (menu.children) {
      menu.children.filter((collapse) => {
        if (collapse.type && collapse.type === 'collapse') {
          ...
          if (document.location.pathname === collapse.path) {
            setMain(menu);
            setItem(collapse);
          }
        }
  ...
  if (main && main.type === 'collapse') {
    CollapseIcon = main.icon ? main.icon : IconSitemap;
    mainContent = (
  ...
  if (item && item.type === 'item') {
    ...
    ItemIcon = item.icon ? item.icon : IconSitemap;
    itemContent = (
      <Typography
        variant="subtitle1"
        sx={{
              ...
              <Grid item>
                ...
                >
                  <Typography
                    ...
                    sx={linkSX}
                  >
                    {icons && <IconSmartHome /* sx={iconStyle} */ />}
                    {icon && <IconHome /* sx={{ ...iconStyle, mr: 0 }} */ />}
                    {!icon && 'Dashboard'}
                  </Typography>
                  {mainContent}
                  {itemContent}
