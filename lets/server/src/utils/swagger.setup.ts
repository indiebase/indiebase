...
export const setupApiDoc = (app: INestApplication) =>
  new Promise(async (resolve) => {
    try {
      ...
      const orgOptions = new DocumentBuilder()
        .setTitle('Organizational structure Api')
        .setDescription('Organizational structure REST API')
        .setVersion('1.0.0')
        .addCookieAuth('SID', {
          type: 'apiKey',
          in: 'cookie',
      ...
      const orgDoc = SwaggerModule.createDocument(app, orgOptions, {
      ...
      SwaggerModule.setup('openapi/org', app, orgDoc, {
        swaggerOptions: {
          persistAuthorization: true,
        },
      });
